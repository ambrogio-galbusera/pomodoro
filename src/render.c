...
void RENDER_DrawSprite(int posx, int posy, int width, int height, const unsigned char* bits) {
	int x, y;
	...
	for (y = 0; y < height; y++) {
		for (x = 0; x < width; x++) {
			...
			alienByte = bits[x / 8 + y * width / 8];
			alienOffset = x % 8;

			/* Mark bits that are coloured */
			if (alienByte & (1 << alienOffset)) {
				framePointer = &frameBuffer[((posx + x) + (posy + y) * SIZE_X)
						/ 8];
...
void RENDER_ClearFramebufferArea(int xstart, int ystart, int xend, int yend,
		int color) {
	int x, y;
	char *framePointer;
	int frameOffset;
...
void RENDER_UpdateDisplay(bool fullUpdate, DISPLAY_Device_t* displayDevice) {
	...
	if (fullUpdate) {
		...
		displayDevice->pPixelMatrixDraw(displayDevice, frameBuffer, 0,
				displayDevice->geometry.width, 0,
				SIZE_Y);
	} else {
		...
		for (i = 0; i < SIZE_Y; i++) {
			if ((dirtyLinesNew[i / 8] & (1 << (i % 8)))
				...
				displayDevice->pPixelMatrixDraw(displayDevice,
						&frameBuffer[startLine * SIZE_X / 8], 0,
						displayDevice->geometry.width, startLine, lineCount);
				lineCount = 0;
			}
		}
